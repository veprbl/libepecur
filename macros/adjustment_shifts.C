...
void	adjustment_shifts()
{
	...
	((TH1*)gDirectory->Get("rl_x"))->GetXaxis()->SetTitle("\\Delta \\text{RL}_x\\text{, мм}");
	c.cd(++pad_id);
	events->Draw("(RL_y-LR_y) >> rl_y", "abs(RL_y - LR_y) < 2");
	((TH1*)gDirectory->Get("rl_y"))->GetXaxis()->SetTitle("\\Delta \\text{RL}_y\\text{, мм}");
	c.cd(++pad_id);
	events->Draw("(RL_z-LR_z) >> rl_z", "abs(RL_z - LR_z) < 30");
	((TH1*)gDirectory->Get("rl_z"))->GetXaxis()->SetTitle("\\Delta \\text{RL}_z\\text{, мм}");

	c.cd(++pad_id);
	events->Draw("(LF2_x-F2L_x) >> lf2_x", "abs(LF2_x - F2L_x) < 1");
	((TH1*)gDirectory->Get("lf2_x"))->GetXaxis()->SetTitle("\\Delta \\text{LF2}_x\\text{, мм}");
	c.cd(++pad_id);
	events->Draw("(LF2_y-F2L_y) >> lf2_y", "abs(LF2_y - F2L_y) < 2");
	((TH1*)gDirectory->Get("lf2_y"))->GetXaxis()->SetTitle("\\Delta \\text{LF2}_y\\text{, мм}");
	c.cd(++pad_id);
	events->Draw("(LF2_z-F2L_z) >> lf2_z", "abs(LF2_z - F2L_z) < 30");
	((TH1*)gDirectory->Get("lf2_z"))->GetXaxis()->SetTitle("\\Delta \\text{LF2}_z\\text{, мм}");

	c.cd(++pad_id);
	events->Draw("(F2R_x-RF2_x) >> f2r_x", "abs(F2R_x - RF2_x) < 1");
	((TH1*)gDirectory->Get("f2r_x"))->GetXaxis()->SetTitle("\\Delta \\text{F2R}_x\\text{, мм}");
	c.cd(++pad_id);
	events->Draw("(F2R_y-RF2_y) >> f2r_y", "abs(F2R_y - RF2_y) < 2");
	((TH1*)gDirectory->Get("f2r_y"))->GetXaxis()->SetTitle("\\Delta \\text{F2R}_y\\text{, мм}");
	c.cd(++pad_id);
	events->Draw("(F2R_z-RF2_z) >> f2r_z", "abs(F2R_z - RF2_z) < 30");
	((TH1*)gDirectory->Get("f2r_z"))->GetXaxis()->SetTitle("\\Delta \\text{F2R}_z\\text{, мм}");
}
